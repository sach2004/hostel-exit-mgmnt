...
const ActivityTable: React.FC = () => {
  ...
  const rejbtn = async (e : React.MouseEvent<HTMLButtonElement>) => {
    const userId = (e.target as HTMLButtonElement).id
  ...
  const accbtn = async (e: React.MouseEvent<HTMLButtonElement>) => {
    const userId =  (e.target as HTMLButtonElement).id;
            ...
            {activities.map((activity) =>
                activity.requests.map((request) =>
                  request.status === "PENDING" ? (
                    <tr key={request.id}>
                      <td className="py-2 px-4 border-b text-center uppercase">
                        {(idx = idx + 1)}
                      </td>
                      <td className="py-2 px-4 border-b text-center uppercase">
                      ...
                      <td className="py-2 px-4 border-b text-center ">
                        <div className="bg-green-500 text-center rounded-lg p-2 text-white uppercase hover:bg-green-600 hover:text-gray-100">
                          <button id={`${request.id}`} onClick={accbtn}>
                            ACCEPT
                          </button>
                        </div>
                      </td>
                      <td className="py-2 px-4 border-b text-center">
                        <div className="bg-red-500 text-center rounded-lg p-2 text-white uppercase hover:bg-red-600 hover:text-gray-100">
                          <button id={`${request.id}`} onClick={rejbtn}>
                            REJECT
                          </button>
                        </div>
                      </td>
